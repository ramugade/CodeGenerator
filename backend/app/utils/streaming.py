...
async def stream_agent_events(
    ...
    """Stream agent state updates as (event_type, data) tuples.
    ...
    Yields:
        Tuples of (event_type, data) for each state change
    """
    ...
    # Cost update event (NEW)
    if current_step in [StepType.PLANNING, StepType.CODE_GENERATION, StepType.ERROR_FIXING]:
        if "token_usage" in state:
            yield ("cost_update", {
                "total_tokens": state.get("total_tokens", 0),
                "estimated_cost_usd": state.get("estimated_cost_usd", 0.0),
                "step_breakdown": state["token_usage"]
            })
